...
describe("BaseDareBountyV2", function () {
  const DARE_ID = 1n;
  ...
  async function deployFixture() {
    ...
    const BaseDareBountyV2 = await ethers.getContractFactory("BaseDareBountyV2");
    const bounty = await BaseDareBountyV2.deploy(await usdc.getAddress(), owner.address);
  ...
  it("allows a zero referrer because referral payouts are paused", async function () {
    const { bounty, backer, streamer } = await deployFixture();

    await expect(
      bounty.connect(backer).fundBounty(DARE_ID, streamer.address, ethers.ZeroAddress, AMOUNT)
    )
      .to.emit(bounty, "BountyFunded")
      .withArgs(DARE_ID, backer.address, AMOUNT);
  });

  it("allows only the referee to pay out and splits funds 96/4/0", async function () {
    ...
    await expect(
      ...
      .withArgs(DARE_ID, 96_000_000n, 4_000_000n, 0n);

    expect(await usdc.balanceOf(streamer.address)).to.equal(streamerBefore + 96_000_000n);
    expect(await usdc.balanceOf(owner.address)).to.equal(ownerBefore + 4_000_000n);
  ...
  it("surfaces the new fee metadata helpers", async function () {
    const { bounty } = await deployFixture();

    expect(await bounty.platformFeePercent()).to.equal(4n);
