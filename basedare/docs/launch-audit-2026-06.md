...
|---|---|---|---|---|---|
| P0-1 | ✅ DONE (`7dda0bc8`) — founder call: KEEP the hero orbit/conveyor visuals (they're the brand). Fix was surgical: real streamer handles on fake funded bounties → fictional Peebear-universe cast (@peebear/@gridghost/@permabear/@whiskerz/@heartbroke/@baldwin). "I dare @…" autocomplete keeps real tags (real offers = the product mechanic, not fake claims). | **Claude** ✅ | `components/HeroEllipticalStream.tsx`, `components/PeeBearConveyor.tsx`, `app/create/page.tsx:78` | `rtk grep "KaiCenat\|xQc"` clean on display comps ✓; eyeball next deploy | Low — shipped clean |
...
| P0-4 | ✅ DONE (`2eeb908a`) — shared `components/SafetyWaiver.tsx` (18+, skill-based-only, assumption of risk, Terms link) gating both "Request to Claim" (`app/dare/[shortId]`) and "Submit Proof" (`SubmitEvidence`). Human: eyeball copy on next deploy. | **Claude** ✅ | `components/SafetyWaiver.tsx`, `components/SubmitEvidence.tsx`, `app/dare/[shortId]/page.tsx` | Checkbox blocks both buttons ✓ tsc/eslint clean | Low — shipped |
