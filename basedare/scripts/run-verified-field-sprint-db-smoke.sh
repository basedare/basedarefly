...
if ! command -v initdb >/dev/null 2>&1; then
  echo "PostgreSQL initdb is required for the Verified Field Sprint database smoke." >&2
  exit 1
fi

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/basedare-sprint-db.XXXXXX")"
...
DB_NAME="basedare_sprint_vertical_$(date +%s)"
...
TS_NODE_COMPILER_OPTIONS='{"module":"CommonJS","moduleResolution":"Node"}' \
  ...
  -r ts-node/register \
  scripts/verified-field-sprint-db-smoke.ts
