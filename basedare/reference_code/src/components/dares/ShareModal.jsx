...
export default function ShareModal({ dare, creator, isOpen, onClose }) {
  ...
  const handleCopy = async () => {
    try {
      // Original logic copied both text and URL, let's keep that structure with the new shareText
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Dare Invite!',
          text: shareText.slice(0, 100), // Instagram share usually prefers shorter text
          url: shareUrl
        });
      } catch (err) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-[rgb(20,20,28)] border-purple-500/30 text-white max-w-md">
        <DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="bg-[rgb(30,30,40)] rounded-lg p-4">
            <p className="text-sm text-gray-300 mb-3 line-clamp-3">{shareText}</p>
            <Input
              value={shareUrl}
              readOnly
              className="bg-[rgb(20,20,28)] border-purple-500/30 text-white text-sm"
            />
          </div>
