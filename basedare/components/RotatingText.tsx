...
type MotionDefinition = Record<string, number | string>;
type MotionTransition = Record<string, number | string>;
...
const RotatingText = forwardRef<RotatingTextRef, RotatingTextProps>((props, ref) => {
  ...
  const activeTextClassName = activeTextClassNames?.[currentTextIndex];

  return (
    <motion.span className={cn(styles.textRotate, mainClassName)} layout transition={transition}>
      <span className={styles.textRotateSrOnly}>{texts[currentTextIndex]}</span>
      <AnimatePresence mode={animatePresenceMode} initial={animatePresenceInitial}>
        <motion.span
          key={currentTextIndex}
          className={cn(
            splitBy === 'lines' ? styles.textRotateLines : styles.textRotateTrack,
            activeTextClassName
          )}
          layout
          aria-hidden="true"
