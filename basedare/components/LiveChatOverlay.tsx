...
import { X, Send, Zap, AlertTriangle } from "lucide-react";
...
export default function LiveChatOverlay({ target, onClose }: ChatOverlayProps) {
  // TABS: 'CHAT' or 'STAKE'
  const [activeTab, setActiveTab] = useState("CHAT");
  
  const [messages, setMessages] = useState([
    { id: 1, user: "SYSTEM", text: `Connected to secure channel: ${target.streamer_name || target.streamer || 'UNKNOWN'}`, color: "text-purple-500", type: "system", isSystem: true, isMe: false }
  ]);
    ...
    const interval = setInterval(() => {
      ...
      setMessages((prev) => [
        ...
        { 
          ...
          type: randomMsg.isSystem ? "system" : "user", 
          isSystem: randomMsg.isSystem ?? false,
          isMe: false 
        }
      ]);
    }, 2500);
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: Date.now(), user: "YOU", text: input, color: "text-white", type: "user", isMe: true, isSystem: false }
    ]);
    setInput("");
  };
  ...
  const handlePledge = () => {
    ...
    setMessages((prev) => [
        ...prev,
        { id: Date.now(), user: "YOU", text: `STAKED ${stakeAmount} ETH ON THE OUTCOME`, color: "text-[#FFD700]", type: "system", isSystem: true, isMe: true }
    ]);
    setActiveTab("CHAT");
    setStakeAmount("");
  };
  ...
  return (
    <motion.div
      ...
      className="fixed inset-y-0 right-0 w-full md:w-[450px] bg-[#050505] border-l border-white/10 flex flex-col shadow-[-50px_0_100px_rgba(0,0,0,0.8)]"
      ...
      <div className="p-6 border-b border-white/10 bg-[#0a0a0a] flex items-center justify-between relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-500 uppercase italic">
            {target.streamer_name || target.streamer || "UNKNOWN"}
          </p>
        </div>
        ...
        {activeTab === "STAKE" && (
           <div className="p-8 h-full flex flex-col items-center">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                     {['0.01', '0.05', '0.1'].map((amt) => (
                        <button 
                           ...
                           className="py-3 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 hover:border-[#FFD700] hover:text-[#FFD700] transition-all font-mono font-bold text-sm"
                        >
                           {amt} ETH
                        </button>
                     ))}
              ...
              <Button 
                  ...
                  className="w-full h-16 bg-[#FFD700] hover:bg-[#FFD700] text-black text-xl font-black uppercase tracking-widest rounded-xl hover:scale-[1.02] transition-transform shadow-[0_0_40px_rgba(255,215,0,0.4)] disabled:opacity-50 disabled:shadow-none"
              >
                 Confirm Pledge
              </Button>
