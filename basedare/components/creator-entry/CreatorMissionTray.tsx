...
export function CreatorMissionTray({ item, className = '' }: { item: CreatorMissionTrayItem; className?: string }) {
  ...
  return (
    <aside
      className={`fixed inset-x-3 bottom-3 z-40 mx-auto max-w-2xl overflow-hidden rounded-[1.35rem] border border-yellow-200/22 bg-[linear-gradient(150deg,rgba(36,27,7,0.97),rgba(6,8,14,0.99)_58%)] p-3 shadow-[0_20px_70px_rgba(0,0,0,0.64),inset_0_1px_0_rgba(255,255,255,0.12)] backdrop-blur-xl ${className}`.trim()}
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-0.5 block truncate text-[10px] font-bold text-white/42">
            {[item.locationLabel, due].filter(Boolean).join(' · ') || 'Open the mission for details'}
          </span>
        </span>
        {payout ? (
