...
import { ArrowLeft, CalendarClock, Check, MapPin, ShieldCheck, Users } from 'lucide-react';
import { useSession } from 'next-auth/react';
import { useCallback, useEffect, useState } from 'react';
import { useAccount, useSignMessage } from 'wagmi';

import PlanShareButton from '@/components/community/PlanShareButton';
import { getMeetupSharePath, getMeetupShareText, type MeetupPlanSummary } from '@/lib/meetup-plan';
...
export default function MeetupPlanClient({ initialPlan }: { initialPlan: MeetupPlanSummary }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (response.ok && payload?.success && payload.data?.plan) {
      ...
      setPlan((current) => ({
        ...next,
        viewerRsvped: next.viewerRsvped || current.viewerRsvped,
      }));
    }
  }, [initialPlan.id]);
    ...
    <main className="relative min-h-screen overflow-hidden px-4 pb-24 pt-28 text-white sm:px-6">
      ...
      <div className="relative mx-auto max-w-xl">
        ...
        <section className="mt-5 overflow-hidden rounded-[2rem] border border-violet-200/18 bg-[linear-gradient(155deg,rgba(28,20,49,0.95),rgba(5,7,14,0.99))] p-5 shadow-[0_28px_90px_rgba(0,0,0,0.54),inset_0_1px_0_rgba(255,255,255,0.09)] sm:p-7">
          <p className="text-[9px] font-black uppercase tracking-[0.22em] text-violet-200/68">Meet me here</p>
          <h1 className="mt-3 text-4xl font-black leading-[0.96] sm:text-5xl">{plan.placeLabel}</h1>
          <div className="mt-5 space-y-2 text-sm font-bold text-white/64">
            ...
            <p className="flex items-center gap-2"><Users className="h-4 w-4 text-violet-200" /> {Math.max(1, plan.rsvpCount)} going</p>
          ...
          {!active ? (
            ...
            <div className="mt-5 flex items-center gap-2 rounded-2xl border border-emerald-200/18 bg-emerald-300/[0.07] p-4 text-sm font-black text-emerald-100"><Check className="h-5 w-5" /> You’re going.</div>
          ) : (
          ...
          <PlanShareButton title={`Meet at ${plan.placeLabel}`} text={getMeetupShareText(plan)} href={getMeetupSharePath(plan.id)} label="Invite mates" className="mt-3 w-full" />
