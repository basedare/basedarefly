...
// THE TARGET DATA YOU WANT TO KEEP
const LIVE_TARGETS = [
  { 
    id: 101, 
    dare: "EAT THE REAPER", 
  ...
  return (
    <section className="py-24 container mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8 perspective-container">
           ...
           {/* RENDER NEW INCOMING DARES FROM DATABASE */}
           {dares.map((newDare) => (
             <div key={newDare.id} className="relative cursor-pointer group">
                <MoltenBountyCard 
                    dare={newDare.description}
                    bounty={`${newDare.stake_amount} USDC`}
                    time="NEW TARGET"
                    streamer={newDare.streamer_name || "@Anon"}
                    imgUrl="https://images.unsplash.com/photo-1614850523296-d8c1af93d400?auto=format&fit=crop&w=800"
                    emoji="🎯"
                    isVulnerable={false}
                />
             </div>
