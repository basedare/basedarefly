...
export function buildFirstNodeDecisionBrief(input: {
  ...
  const question = `Can one funded, time-bounded mission produce accepted place evidence at ${input.venueName} during an agreed test window?`;
  ...
  return {
    ...
    question,
    action:
      'Agree one test window and one observable place question. BaseDare routes the funded mission through the existing claim, proof-review, and payout rail.',
    evidenceRequired:
    ...
    budget:
      'Budget is confirmed in the close room. Nothing launches until the managed-service fee and contributor reward pool are explicitly approved and funded.',
    deliverables: [
      'One approved venue question',
      'One funded mission route',
      'Reviewed evidence and payout state',
      'One buyer receipt with a repeat, adjust, or stop recommendation',
    ],
    nonPromises: [
