...
import {
  buildProofSubmitMessage,
  isProofSubmitFresh,
} from '@/lib/proof-submit-auth';
...
async function getVerifiedWalletSignature(
  ...
  const walletHeader = request.headers.get('x-basedare-proof-wallet')?.trim() ?? null;
  const signature = request.headers.get('x-basedare-proof-signature')?.trim() ?? null;
  const issuedAt = request.headers.get('x-basedare-proof-issued-at')?.trim() ?? null;
  ...
  if (!isProofSubmitFresh(issuedAt)) {
    return null;
  }

  try {
    const message = buildProofSubmitMessage({
...
export async function getAuthorizedProofSubmitterWallet(
  request: NextRequest,
  {
    dareId,
    authorizedWallets,
