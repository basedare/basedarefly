...
import {
  buildDareImageUploadMessage,
  isDareImageUploadFresh,
} from '@/lib/dare-image-auth';
...
async function getVerifiedWalletSignature(
  ...
  const walletHeader = request.headers.get('x-basedare-dare-image-wallet')?.trim() ?? null;
  const signature = request.headers.get('x-basedare-dare-image-signature')?.trim() ?? null;
  const issuedAt = request.headers.get('x-basedare-dare-image-issued-at')?.trim() ?? null;
  ...
  if (!isDareImageUploadFresh(issuedAt)) {
    return null;
  }
...
export async function getAuthorizedDareImageWallet(
  request: NextRequest,
  expectedWalletAddress: string
): Promise<string | null> {
