...
import {
  buildCreatorReviewMessage,
  isCreatorReviewFresh,
} from '@/lib/creator-review-auth';
...
async function getVerifiedWalletSignature(
  ...
  const walletHeader = request.headers.get('x-basedare-review-wallet')?.trim() ?? null;
  const signature = request.headers.get('x-basedare-review-signature')?.trim() ?? null;
  const issuedAt = request.headers.get('x-basedare-review-issued-at')?.trim() ?? null;
  ...
  if (!isCreatorReviewFresh(issuedAt)) {
    return null;
  }
...
export async function getAuthorizedCreatorReviewWallet(
  request: NextRequest,
  expectedWalletAddress: string,
  dareId: string
): Promise<string | null> {
  ...
  return getVerifiedWalletSignature(request, normalizedWallet, dareId);
}
