...
- Reseeding remains an explicit admin operation if the underlying untouched live rows should move to v3; deployment alone safely updates their public presentation.
- Files owned in this pass: `app/dare/[shortId]/*`, the Community Spark presentation branches in `app/api/dare/[shortId]`, `app/api/dares`, and `app/api/dares/nearby`, `components/SafetyWaiver.tsx`, and `lib/action-sports-community-sparks*`.
- No schema, migration, payout, proof decision, claim authorization, map renderer, or production data changed.
...
### Codex Local Partner alignment (2026-07-11)
- `/hosts` is now an **optional Local Partner** intake for authorized routes, gatherings, place handshakes, and local operations. It explicitly says ordinary contributors do not apply there; they find a paid mission on the map and build reputation through completed work.
- Public screening no longer asks for audience size, creator lane, social platform, content style, or venue activation ability. It asks for local capability, relevant experience, a safe support plan, access, availability, operator fee, and explicit authorization.
- `POST /api/creator-captains` now accepts an `applicationKind` discriminator so the new `local_partner` form and Claude-owned legacy creator intake can safely share the existing admin queue. Telegram alerts distinguish the two application types.
- Claude-owned homepage and creator surfaces still contain prominent `Become a Host` CTA labels. **Follow-up for Claude:** demote/relabel those links toward `Find paid missions`, keeping Local Partner as a secondary optional path. Codex did not edit Claude-owned `app/page.tsx`, `components/home/*`, or `components/creators/*`.
