...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: 'Unable to load saved missions.' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: 'Unable to forget saved missions.' }, { status: 500 });
  }
}
