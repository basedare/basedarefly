...
const ActivationAttributionSchema = z
  .object({
    ...
    buyerWalletAddress: z.string().regex(/^0x[a-fA-F0-9]{40}$/).optional().nullable(),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const activationAttribution = {
      ...
      buyerWalletAddress: normalizeText(input.activationAttribution.buyerWalletAddress || '').toLowerCase(),
