...
function FieldSprintsAdminContent() {
  ...
  const [replacementDrafts, setReplacementDrafts] = useState<Record<string, { dareId: string; kind: 'REJECTED' | 'ABANDONED'; reason: string; funding: 'RECOVERED_ESCROW' | 'SUPPLEMENTAL_125'; reference: string }>>({});
  ...
  const preflight = useMemo(() => preflightMissionKit({
    kitKey: form.missionKitKey,
    ...
    <main className="min-h-screen bg-[#07070b] px-4 pb-24 pt-24 text-white sm:px-6">
      <div className="mx-auto max-w-7xl">
        ...
        {(address || hasAdminSession) ? <form onSubmit={createSprint} className="mt-8 grid gap-4 rounded-3xl border border-[#ffe36a]/15 bg-gradient-to-br from-[#241d0b]/55 to-[#101018] p-5 md:grid-cols-2">
          ...
          <button disabled={busy || form.stationLinkIds.length !== 2 || !preflight.ok} className="md:col-span-2 h-12 rounded-xl bg-[#f5c518] text-xs font-black uppercase tracking-[0.16em] text-black disabled:opacity-40">Compile four checked contracts</button>
        </form> : null}
        ...
        <section className="mt-10 space-y-5">{sprints.map((sprint) => {
          ...
          return <article key={sprint.id} className="rounded-3xl border border-white/10 bg-white/[0.035] p-5">
            <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#62efff]">{sprint.status} · {sprint.campaignCode}</p><h2 className="mt-2 text-2xl font-black">{sprint.buyerQuestion}</h2><p className="mt-2 text-sm text-white/45">{sprint.buyerOrganization || sprint.buyerName} · {sprint.areaLabel} · refresh every {sprint.freshnessWindowHours}h · {sprint.missions[0]?.outcomeContractSnapshot?.missionKit?.label ?? 'Legacy custom brief'}</p></div>{sprint.status === 'COMPLETE' ? <a href={`/field-sprints/${sprint.receiptCode}`} target="_blank" className="inline-flex items-center gap-2 rounded-xl border border-emerald-300/20 bg-emerald-300/10 px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-emerald-100">Receipt <ExternalLink className="h-4 w-4" /></a> : null}</div>
            ...
            {sprint.buyerDecisions.length ? <div className="mt-4 rounded-xl border border-emerald-300/15 bg-emerald-300/[0.05] p-3"><b className="text-[10px] uppercase tracking-[0.14em] text-emerald-100">Buyer decisions</b>{sprint.buyerDecisions.map((decision) => <p key={decision.id} className="mt-2 text-xs text-white/55"><b>{decision.decision}</b> · {decision.contactName || 'anonymous'}{decision.nextQuestion ? ` · ${decision.nextQuestion}` : ''}{decision.note ? ` · ${decision.note}` : ''}</p>)}</div> : null}
            ...
            <div className="mt-5 grid gap-3 lg:grid-cols-4">{sprint.missions.map((mission) => {
              ...
              const replacement = replacementDrafts[missionKey] ?? { dareId: '', kind: 'REJECTED' as const, reason: '', funding: 'SUPPLEMENTAL_125' as const, reference: '' };
              const selectedPlace = missionPlaces[missionKey];
              const canReplace = mission.links.length === 1 && ['COLLECTING','REVIEW'].includes(sprint.status) && (mission.status === 'REJECTED' || mission.dare?.status === 'REFUNDED');
              return <div key={mission.id} className="rounded-2xl border border-white/10 bg-black/35 p-4"><div className="flex items-center justify-between"><b>Mission {mission.ordinal}</b><span className="text-[9px] font-black text-[#62efff]">{mission.status}</span></div><p className="mt-2 text-xs text-white/40">$125 gross · $120 net · Field Truth v1 · {mission.links.length || 0} escrow link{mission.links.length === 1 ? '' : 's'}</p>{mission.dare ? <p className="mt-3 text-xs text-white/65">{mission.dare.title}<small className="block text-white/30">{mission.dare.status} · {mission.dare.shortId || mission.dareId}</small></p> : sprint.status === 'ROUTING' ? <div className="mt-3"><MissionPlacePicker value={selectedPlace} onSelect={(place) => setMissionPlaces((current) => { const next = { ...current }; if (place) next[missionKey] = place; else delete next[missionKey]; return next; })} />{selectedPlace ? <Link href={buildMissionCreateHref(sprint, mission.ordinal, selectedPlace)} className="mt-2 flex h-10 w-full items-center justify-center rounded-lg bg-[#f5c518] text-[10px] font-black uppercase tracking-[0.12em] text-black">Fund this $125 mission</Link> : <div className="mt-2 flex h-10 items-center justify-center rounded-lg border border-white/10 text-[9px] font-black uppercase tracking-[0.1em] text-white/30">Choose the exact place first</div>}<details className="mt-2"><summary className="cursor-pointer text-[9px] font-black uppercase tracking-[0.1em] text-white/35">Repair an existing escrow link</summary><input placeholder="Real Dare database ID" value={missionDrafts[missionKey] ?? ''} onChange={(event) => setMissionDrafts({ ...missionDrafts, [missionKey]: event.target.value })} className="mt-2 h-10 w-full rounded-lg border border-white/10 bg-black px-3 text-xs" /><button onClick={() => void act({ action: 'LINK_MISSION', sprintId: sprint.id, ordinal: mission.ordinal, dareId: missionDrafts[missionKey] })} className="mt-2 h-9 w-full rounded-lg border border-[#62efff]/25 bg-[#62efff]/10 text-[10px] font-black text-[#9af5ff]">LINK EXISTING ESCROW</button></details></div> : null}{mission.evidenceQuality ? <p className="mt-3 text-xs">Evidence: <b>{mission.evidenceQuality}</b> · payout {mission.contributorPayoutUsd ?? 'pending'}</p> : null}{canReplace ? <details className="mt-3 rounded-xl border border-amber-300/20 bg-amber-300/[0.05] p-3"><summary className="cursor-pointer text-[9px] font-black uppercase tracking-[0.12em] text-amber-100">Replace once — preserve first attempt</summary><div className="mt-3 space-y-2"><input value={replacement.dareId} onChange={(event) => setReplacementDrafts({ ...replacementDrafts, [missionKey]: { ...replacement, dareId: event.target.value } })} placeholder="New real $125 Dare ID" className="h-9 w-full rounded-lg border border-white/10 bg-black px-2 text-xs" /><select value={replacement.kind} onChange={(event) => setReplacementDrafts({ ...replacementDrafts, [missionKey]: { ...replacement, kind: event.target.value as 'REJECTED' | 'ABANDONED' } })} className="h-9 w-full rounded-lg border border-white/10 bg-black px-2 text-xs"><option value="REJECTED">Rejected attempt</option><option value="ABANDONED">Abandoned + refunded</option></select><textarea value={replacement.reason} onChange={(event) => setReplacementDrafts({ ...replacementDrafts, [missionKey]: { ...replacement, reason: event.target.value } })} placeholder="Specific reason (no personal data)" className="min-h-16 w-full rounded-lg border border-white/10 bg-black p-2 text-xs" /><select value={replacement.funding} onChange={(event) => setReplacementDrafts({ ...replacementDrafts, [missionKey]: { ...replacement, funding: event.target.value as 'RECOVERED_ESCROW' | 'SUPPLEMENTAL_125' } })} className="h-9 w-full rounded-lg border border-white/10 bg-black px-2 text-xs"><option value="SUPPLEMENTAL_125">Separate supplemental $125</option><option value="RECOVERED_ESCROW">Recovered refunded escrow</option></select><input value={replacement.reference} onChange={(event) => setReplacementDrafts({ ...replacementDrafts, [missionKey]: { ...replacement, reference: event.target.value } })} placeholder="Refund / top-up reference" className="h-9 w-full rounded-lg border border-white/10 bg-black px-2 text-xs" /><button onClick={() => void act({ action: 'REPLACE_MISSION', sprintId: sprint.id, ordinal: mission.ordinal, dareId: replacement.dareId, replacementKind: replacement.kind, replacementReason: replacement.reason, fundingTreatment: replacement.funding, fundingReference: replacement.reference })} className="h-9 w-full rounded-lg border border-amber-300/25 bg-amber-300/10 text-[9px] font-black text-amber-100">RECORD + LINK ONE REPLACEMENT</button></div></details> : null}{['COLLECTING','REVIEW'].includes(sprint.status) ? <div className="mt-3 grid grid-cols-2 gap-2"><input value={review.minutes} onChange={(event) => setReviewDrafts({ ...reviewDrafts, [missionKey]: { ...review, minutes: event.target.value } })} placeholder="minutes" className="h-9 rounded-lg border border-white/10 bg-black px-2 text-xs" /><input value={review.cost} onChange={(event) => setReviewDrafts({ ...reviewDrafts, [missionKey]: { ...review, cost: event.target.value } })} placeholder="$ cost" className="h-9 rounded-lg border border-white/10 bg-black px-2 text-xs" /><button onClick={() => void act({ action: 'RECORD_REVIEW_COST', sprintId: sprint.id, ordinal: mission.ordinal, reviewMinutes: Number(review.minutes), reviewCostUsd: Number(review.cost) })} className="col-span-2 h-8 rounded-lg border border-white/10 text-[9px] font-black">SAVE REVIEW COST</button></div> : null}</div>;
            })}</div>
